...
{
    info_wtime(log_fp, "rc_revoke_log_access() start\n", SID_GET_TERM(SRV_DATA->ctrl_data->sid));
    int rc;
    ...
    for (i = 0; i < size; i++) {
        ...
        // rc_qp_reset_to_rts(ep, LOG_QP);
        rc_print_qp_state(ep);

        // info_wtime(log_fp, "qp flags start\n", SID_GET_TERM(SRV_DATA->ctrl_data->sid));
        
        // struct ibv_qp_attr attr;
        // memset(&attr, 0, sizeof(attr));
        // attr.qp_access_flags = IBV_ACCESS_LOCAL_WRITE;
        // int ret = ibv_modify_qp(ep->rc_ep.rc_qp[LOG_QP].qp, &attr, IBV_QP_ACCESS_FLAGS);
        // if (0 != ret) {
        //     info_wtime(log_fp, "111\n", SID_GET_TERM(SRV_DATA->ctrl_data->sid));
        //     rc = rc_qp_reset(ep, LOG_QP);
        //     if (0 != rc) {
        ...
        // info_wtime(log_fp, "qp flags end\n", SID_GET_TERM(SRV_DATA->ctrl_data->sid));
        
        info_wtime(log_fp, "qp state transfer start\n", SID_GET_TERM(SRV_DATA->ctrl_data->sid));
        rc = rc_qp_reset(ep, LOG_QP);
        if (0 != rc) {
            error_return(1, log_fp, "Cannot revoke the access of LID=%"PRIu16"\n",
                         ep->ud_ep.lid);
        }
        info_wtime(log_fp, "qp state transfer end\n", SID_GET_TERM(SRV_DATA->ctrl_data->sid));
        ep->log_access = 0;
    }
...
{
    info_wtime(log_fp, "rc_restore_log_access start\n", SID_GET_TERM(SRV_DATA->ctrl_data->sid));
    int rc;
