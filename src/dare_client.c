    ...
    switch(type) {
        ...
        case CSM_REPLY:
            if (CLT_TYPE_LOOP == data.input->clt_type) {
                ...
                long latency_us = (end.tv_sec - req_start.tv_sec) * 1000000L + (end.tv_usec - req_start.tv_usec);
                printf("[E2E] Latency: %ld us\n", latency_us);
            ...
            if (CLT_TYPE_RTRACE == data.input->clt_type) {
                ...
                // 计算端到端时延
                gettimeofday(&end, NULL);
                last_latency_us = (end.tv_sec - req_start.tv_sec) * 1000000L + (end.tv_usec - req_start.tv_usec);
                measure_count++;
                repeat_last_cmd = 0;
