...
poll_vote_requests()
{
     ...
     * We'll perform priority-based selection: first determine the highest
     * priority among candidates (including self), then among those with
     * the highest priority choose the most up-to-date log (as before).
     */
    ...
    /* Revoke remote access to local log; need exclusive access to get last entry */
    rc = dare_ib_revoke_log_access();
    if (0 != rc) {
        /* This should never happen */
    ...
    /* Create not committed buffer & get local last entry for comparison */
...
//text(log_fp, "\n   Local [idx=%"PRIu64"; term=%"PRIu64"]\n", best_request.index, best_request.term);        
    info(log_fp, "   # Local [idx=%"PRIu64"; term=%"PRIu64"]\n", 
            best_request.index, best_request.term);
    /* First: filter candidates by log freshness (must be at least as up-to-date
     * as local). Among those, consider only candidates with priority > my_priority.
     * If multiple such candidates exist, choose the one with highest priority
     * (tie-breaker: fresher log).
     */
    uint64_t highest_term = SID_GET_TERM(best_request.sid);
    int found_fresh = 0;
    int have_priority_candidate = 0;
    vote_req_t priority_choice;
    for (i = 0; i < size; i++) {
        request = &(data.ctrl_data->vote_req[i]);
        if (i == data.config.idx) continue;
        if (request->sid == 0) continue;
        info(log_fp, "   # Remote(p%"PRIu8") [idx=%"PRIu64"; term=%"PRIu64"]\n", 
                    i, request->index, request->term);

        /* Freshness test: candidate's log must be >= local */
        if ( (request->term < best_request.term) ||
             ((request->term == best_request.term) && (request->index < best_request.index)) )
        {
            /* Candidate's log is not up-to-date; drop request */
            request->sid = 0;
            continue;
        }
        found_fresh = 1;
        ...
        /* Update freshest candidate (used if we don't vote due to priority) */
        if ( (request->term > best_request.term) ||
             ((request->term == best_request.term) && (request->index > best_request.index)) )
        {
        ...
        if (request->priority > target_priority) {
            ...
            else if (request->priority > priority_choice.priority ||
                     (request->priority == priority_choice.priority &&
                      (request->term > priority_choice.term ||
                       (request->term == priority_choice.term && request->index > priority_choice.index))))
            {
                priority_choice = *request;
            }
        }
        /* Clear processed request to avoid reusing it */
        request->sid = 0;
    }

    /* If there was no candidate with fresher-or-equal log, nothing to do */
    if (!found_fresh) {
        TIMER_STOP(log_fp);
        return;
    }

    /* If no candidate had priority > my_priority, do not vote; instead bump term */
    if (!have_priority_candidate) {
        new_sid = data.ctrl_data->sid;
